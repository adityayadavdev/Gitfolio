...
// .wrangler/tmp/bundle-8rBWsR/checked-fetch.js
...
async function createOrderHandler(request, env) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return new Response(`Razorpay API error: ${error}`, { status: response.status });
    }
...
async function licenseValidateHandler(request, env) {
  ...
  try {
    ...
    if (licenseData) {
      const { plan, createdAt } = JSON.parse(licenseData);
      return new Response(JSON.stringify({ valid: true, plan, createdAt }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      });
    }
...
async function aiHandler(request, env) {
  ...
  try {
    ...
    const currentUsage = parseInt(await env.KV.get(rateLimitKey) || "0");
    ...
    await env.KV.put(rateLimitKey, (currentUsage + 1).toString(), { expirationTtl: 86400 });
    const promptCfg = PROMPTS[feature];
    const systemPrompt = promptCfg.system;
    const userPrompt = promptCfg.user(payload);
    let result;
    if (isPremium) {
      result = await callClaude({ system: systemPrompt, user: userPrompt }, env);
    } else {
      const llamaRes = await env.AI.run("@cf/meta/llama-3.3-70b-instruct-fp8-fast", {
        prompt: `${systemPrompt}

${userPrompt}`
      });
      result = llamaRes.response;
    }
    return new Response(JSON.stringify({ result, model, isPremium }), {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (method === "POST" && path === "/auth/device") {
        ...
        response = await aiHandler(request, env, ctx);
...
// .wrangler/tmp/bundle-8rBWsR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-8rBWsR/middleware-loader.entry.ts
