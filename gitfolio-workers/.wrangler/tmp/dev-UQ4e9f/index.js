...
// .wrangler/tmp/bundle-HVqoPY/checked-fetch.js
...
async function licenseValidateHandler(request, env) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ valid: true, plan: "lifetime", createdAt: (/* @__PURE__ */ new Date()).toISOString() }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
  } catch (e) {
...
async function aiHandler(request, env) {
  ...
  try {
    ...
    const { content, modelUsed } = await callClaude({ system: systemPrompt, user: userPrompt }, env, modelList);
...
// .wrangler/tmp/bundle-HVqoPY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HVqoPY/middleware-loader.entry.ts
